...
jit_function_t build_unrolled_dot_product(jit_context_t context, unsigned int r) {
    ...
    // temporary values
    jit_value_t i, z, xir, yir, tmp, tmp1, tmp2;
    tmp = jit_value_create(F, jit_type_nint);
    i = jit_value_create(F, jit_type_nint);
    z = jit_value_create(F, jit_type_float64);
    xir = jit_value_create(F, jit_type_float64);
    yir = jit_value_create(F, jit_type_float64);
    tmp1 = jit_value_create(F, jit_type_float64);
    tmp2 = jit_value_create(F, jit_type_float64);
    ...
    // Make the instructions for the loop contents
    for (k = 0; k < r; k++) {
        // Retrieve x[i],...,x[i+r-1], y[i],...,y[i+r-1]
        xir = jit_insn_load_elem(F, x, i, jit_type_float64);
        yir = jit_insn_load_elem(F, y, i, jit_type_float64);

        // Multiply xir, yir and add the result to z
        tmp1 = jit_insn_mul(F, xir, yir);
        tmp2 = jit_insn_add(F, z, tmp1);
        jit_insn_store(F, z, tmp2);
